...
//El vector puede estar tan sobredimensionado como se desee. Pero al menos debe contar con un componente adicional para guardar en él el valor nulo.
...
/*La funcion strcpy() recibe dos parametros: un vector de caracteres y una cadena de caracteres, y sirve para copiar dicha cadena en el vector. Es equivamente 
en Python a simplemente declarar una variable tipo string y asignarle un valor. Esta funcion agrega el valor nulo luego de la cadena de caracteres.*/
...
int main(int argc,char *argv[]){
    ...
    *P = 3;
    ...
    Como dato adicional: si queremos emplear la operacion de incremento "++" o decremento "--" con un puntero, deberemos emplear la sintaxis (*p)++, porque si ponemos *p++ nos va
...
void asignacion_vector(int *puntero, int elementos)

//Asi es como pasamos un vector como parametro con este metodo.

puntero[i]

/*En el cuerpo de la funcion vamos a trabajar con el puntero como si fuera un vector, por lo que habiamos dicho de que puntero y vector son lo mismo. Porque el vector ya era de por si una direccion
de memoria, la del primer elemento.

Matriz:

El pasaje de matrices como parametros por referencia es un poco mas complejo. El metodo se basa en entender a la matriz como realmente se ve en la memoria: una secuencia lineal de
elementos (columnas) organizados en grupos (filas), en lugar de visualizar una tabla. Pensandolo asi, podriamos acceder a cualquier elemento de la matriz solo con conocer la direccion
de memoria de su primer elemento (m[0][0]), a traves de lo que se conoce como "formula de direccionamiento".*/
...
int main(int argc,char *argv[]){
    ...
    //Lo que interesa en la funcion main es que guardemos en un puntero la direccion de memoria del primer elemento de la matriz luego de definirla, de esta manera.

    asignacionMatriz(p,3,3);

    // Esta direccion es la que vamos a pasar como parametro en la funcion que recibe una matriz, junto al tamaño de sus filas y columnas.

    imprimirMatriz(p,3,3);
